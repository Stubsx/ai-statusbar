...
灵眸是一个 macOS 菜单栏工具，用来汇总本机 AI 编程工具的运行状态、最近任务、Token 用量和配额。目前支持 Codex App/CLI、Kimi Code、Kimi Work、Claude Code、Hermes、ZCode 和 DSH（DeepSeek Harness 网页端）。
...
- 看板收起后的浮球支持晴蓝、水墨两种风格，在「设置 → 通用 → 浮球风格」切换；两种风格均由 Swift 程序绘制圆润头型、细微笔触、浓淡叠染和透明渗边，不依赖图片素材。晴蓝以天蓝与浅蓝水洗叠色，水墨使用浓墨与枯笔工作环，均保留眨眼、视线跟随和状态数字。色层缓缓流动，渗墨与枯笔纹理沿轮廓轻柔游移；水墨空闲时 14 秒、工作时 8 秒一循环；晴蓝使用更明显的色层流动，空闲时 10 秒、工作时 6 秒一循环；动画帧在后台生成并缓存，由原生图层播放，隐藏时暂停，休眠时色层静止，开启系统「减少动态效果」时使用静态画面。
...
- 轻量状态速览：每个工具只显示一条当前任务，等待回答优先提醒；最近事件按需查看，本机保留 7 天 / 200 条。
- 点击事件打开原应用或宿主，并清除这次未读提醒，保留历史；连接诊断解释未运行、未发现、数据失败与配额过期。
- 可选合并提醒、暂时静音、演示模式隐藏标题、低配额及恢复提醒。
- 默认 300 点宽的状态浮窗，任务增加不会撑大面板；用量、活跃、配额、记录与运行详情收在右上角菜单。支持登录时启动和手动检查正式版本。
- 桌宠 ZIP 导出、作者与授权元数据、素材校验、创作模板及本地作品目录。
- 可选本地事件订阅，可接入快捷指令、Raycast 与脚本；默认关闭，不自动执行动作。
- 可选 Dock 图标和背景自适应外观。
- 缺少某个工具或本地数据格式不兼容时，不影响其他工具运行。

体验路线图与验收记录见 [产品计划](docs/PRODUCT_ROADMAP.md)。详细说明：[工具支持矩阵](docs/TOOLS.md) · [适配贡献](docs/ADAPTERS.md) · [桌宠分享](docs/PETS.md) · [本地事件接口](docs/LOCAL_EVENTS.md)。
...
- **通知**：仅在用户启用任务更新或配额提醒后申请。任务标题可能出现在通知中心或锁屏上。
...
- **用量同步**：默认关闭。开启后仅把“日期、工具名、Token 计数”写入用户自选的同步目录（默认 iCloud Drive），供自己的其他设备读取合并；不包含任务标题、会话内容或账号信息。目录内容随所用网盘服务同步，请勿与他人共用同一目录。
...
产品迭代的优先级、实施范围和验收标准见 [产品体验迭代计划](docs/PRODUCT_ROADMAP.md)。
...
原生 App 的会话跟踪回归可单独运行 `bash scripts/test-busy-sessions.sh`，也已纳入 `check-open-source.sh`。任务连续离开完整的忙碌列表三轮后才推送提醒；列表截断时暂停判定，工具离线或采集失败后重新建立跟踪基线。
...
- 任务标题来自本机工具，可能包含敏感项目名称；可以开启演示模式隐藏标题，或在系统中关闭通知预览。
