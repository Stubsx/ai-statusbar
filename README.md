...
- 按 Harness 只展示运行中的会话和未查看的新结果，同一对话只显示一次；会话直接平铺，运行与待查看数量和小图标一致。点击会话跳转后同步移除已读结果与提醒；本机历史保留 7 天 / 200 条，不混入状态列表。
- 点击会话直达支持的对话，或打开原应用与宿主，同时清除这条会话已显示的未读提醒；连接诊断解释未运行、未发现、数据失败与配额过期。
- 可选合并提醒、暂时静音、演示模式隐藏标题、低配额及恢复提醒。
- 状态、用量、活跃、配额四页统一为 380 点宽，任务增加时在面板内滚动。顶栏平铺「状态、用量、活跃、配额」四个小按钮，一次露出约三个半，溢出边缘以磨砂玻璃渐隐，支持 macOS 左右滚动和单击切换；⌘1–⌘4 直接选页。支持登录时启动和手动检查正式版本。
